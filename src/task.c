...
static void clock_tick_handler(registers_t regs)
{
  // Switch task on every tick
  switch_task();
}
...
void initialise_tasking()
{
    ...
    __clk_setup_tick_handler(CLOCK_MONOTONIC, clock_tick_handler, __CLK_TASKING_HANDLER_ID);
    ...
    asm volatile(
      ...
      "jmp *%%ecx           "
        ::  "g"(eip),
            "g"(esp),
            "g"(ebp),
            "g"(current_directory->physicalAddr));
}
